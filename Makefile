...
var_to_obj=$(if $($1_SRCS),$(OBJDIR)/$(call lower_case,$1).o)
var_to_hdr=$(if $($1_HDRS),$(INCDIR)/$(call lower_case,$1).h)
ifeq ($(CONFIG_ECL_TEST),y)
var_to_test=$(if $($1_TEST),$(BINDIR)/$(call lower_case,$1).test)
else
var_to_test=
endif
...
all : $(OUTPUT) $(BINDIR) $(LIBDIR) $(INCDIR) $(OBJDIR) start_header $(FINAL_TARGETS) finalize_header
	@echo
...
$(OBJDIR)/%.o:
	$(CC) $(CFLAGS) -c $(call obj_to_src,$(@F)) -o $@ -I$(call obj_to_folder,$(@F)) -I$(INCDIR)
...
$(BINDIR)/%.test:
	$(CC) $(CFLAGS) -o $@ $(subst _,/,$(subst .test,,$(@F)))/test.c $(OBJDIR)/$(subst -test,,$(subst .test,,$(@F))).o
...
$(TOOLS) $(OUTPUT) $(BINDIR) $(LIBDIR) $(INCDIR) $(OBJDIR): 
	@mkdir -p $@
